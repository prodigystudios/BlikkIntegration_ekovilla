...
import { formatDateTimeStockholm, isoWeekKey, isWeekend, getSwedishPublicHolidays, isSwedishHoliday } from '../_lib/date';
                        ...
                        return (
                          ...
                          >
                            {!isDelivery && isStart && pos >= 0 && (
                              <span style={{ position: 'absolute', top: 6, right: 6, background: '#ffffffd9', color: '#0f172a', fontSize: 9, fontWeight: 700, padding: '2px 6px', borderRadius: 999, border: '1px solid #cbd5e1' }} title="Placering i dag/lastbil">
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                              <span style={{ fontWeight: 600, color: isDelivery ? (isDeliveryOutbound ? '#3a2200' : '#18065fff') : (display ? display.text : '#312e81'), display: 'flex', alignItems: 'center', columnGap: 6, rowGap: 2, flexWrap: 'wrap' }}>
                                ...
                                <span title={it.project.name} style={{ color: isDelivery ? '#ffffffff' : (display ? display.text : '#e7e7eeff'), fontWeight: 700, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '100%' }}>{it.project.name}</span>
                                {isDelivery && (
                              ...
                              {!isDelivery && isStart && (
                                (() => {
                                  ...
                                  return names.length > 0 ? (
                                    <span style={{ fontSize: 8.5, color: display ? display.text : '#e1e3e5ff', background: '#ffffff50', padding: '2px 7px', borderRadius: 999, border: `1px solid ${cardBorder}44` }} title={`Team: ${names.join(', ')}`}>
                                      Team: {names.join(', ')}
                                    </span>
                                  ) : null;
                                })()
                              ...
                              {(!isDelivery || isDeliveryOutbound) && isStart && <span style={{ color: display ? display.text : '#6366f1', fontSize: 9.5, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={it.project.customer}>{it.project.customer}</span>}
                              {/* Show project badges */}
                              {(!isDelivery || isDeliveryOutbound) && isStart && (() => {
                                ...
                                return (
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', paddingTop: 2 }}>
                                    {projectStatus ? <span style={{ fontSize: 8.5, color: textColor, background: bg, padding: '3px 7px', borderRadius: 999, border: `1px solid ${border}`, fontWeight: 700 }}>{projectStatus}</span> : null}
                                    {clientNotified ? (
                                      <span title={clientNotifiedBy ? `Kund notifierad av ${clientNotifiedBy}${clientNotifiedAt ? ` ${clientNotifiedAt}` : ''}` : 'Kund notifierad'} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 8.5, background: '#d1fae5', color: '#065f46', padding: '3px 7px', borderRadius: 999, border: '1px solid #6ee7b7', fontWeight: 700 }}>
                                    ...
                                    {smsNotified ? (
                                      <span title={smsNotifiedBy ? `SMS skickat av ${smsNotifiedBy}${smsNotifiedAt ? ` ${smsNotifiedAt}` : ''}${smsDeliveryStatus ? ` (${smsDeliveryStatus})` : ''}` : 'SMS skickat'} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 8.5, background: '#dbeafe', color: '#1d4ed8', padding: '3px 7px', borderRadius: 999, border: '1px solid #93c5fd', fontWeight: 700 }}>
                                        <span style={{ display: 'inline-grid', placeItems: 'center', width: 10, height: 10, borderRadius: 999, background: '#2563eb', color: '#fff', fontSize: 8, fontWeight: 800 }}>✓</span>
                                        SMS
                                      </span>
                                    ) : null}
                                    {!smsNotified && smsError ? (
                                      <span title={`SMS misslyckades${smsError ? `: ${smsError}` : ''}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 8.5, background: '#fff7ed', color: '#9a3412', padding: '3px 7px', borderRadius: 999, border: '1px solid #fdba74', fontWeight: 700 }}>
                                        SMS fel
                                      </span>
                                    ) : null}
                              ...
                              {!isDelivery && isStart && it.project.salesResponsible && <span style={{ fontSize: 8.5, color: display ? display.text : '#334155', background: '#ffffff40', padding: '2px 7px', borderRadius: 999, border: `1px solid ${cardBorder}44` }}>Sälj: {it.project.salesResponsible}</span>}
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 6, marginTop: 2 }}>
                                  ...
                                  {isStart && (
                                    <button
                                      ...
                                      style={{ fontSize: 8.5, background: '#fff7ed', border: '1px solid #fdba74', color: '#9a3412', borderRadius: 999, padding: '2px 7px', marginRight: 0, textTransform: 'none' }}
                                    >
                                      Pausa
                                    </button>
                                  )}
                                  <button
                                    ...
                                    style={{ fontSize: 8.5, background: '#ecfdf5', border: '1px solid #6ee7b7', color: '#047857', borderRadius: 999, padding: '2px 7px', fontWeight: 600 }}
                                  >
                                    Lägg till dag
                                  </button>
                                  </div>
