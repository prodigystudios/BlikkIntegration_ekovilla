...
const BASE = { hasSubscription: false, permission: 'default' as PushPermission, optedOut: false, promptDismissed: false };

describe('decidePushSync — de fyra grenarna', () => {
  it('lokal prenumeration finns → persist, oavsett allt annat', () => {
    for (const permission of ['default', 'granted', 'denied'] as PushPermission[]) {
      for (const optedOut of [false, true]) {
        for (const promptDismissed of [false, true]) {
          expect(
            decidePushSync({ hasSubscription: true, permission, optedOut, promptDismissed }),
          ).toBe('persist');
        }
      }
    }
  ...
  it('ingen prenumeration + granted → tyst omprenumeration', () => {
    expect(decidePushSync({ ...BASE, permission: 'granted' })).toBe('resubscribe-silently');
  });
...
describe('decidePushSync — avfärdande och av-val', () => {
  ...
  it('ett avfärdande stoppar INTE den tysta omprenumerationen — den syns ju inte', () => {
    expect(decidePushSync({ ...BASE, permission: 'granted', promptDismissed: true })).toBe('resubscribe-silently');
  });
  ...
  it('av-val slår tyst omprenumeration — annars slås notiser på igen efter disable()', () => {
    ...
    expect(decidePushSync({ ...BASE, permission: 'granted', optedOut: true })).toBe('idle');
  });
  ...
  it('varje kombination ger exakt ett giltigt utfall', () => {
    ...
    for (const hasSubscription of [false, true]) {
      for (const permission of ['default', 'granted', 'denied'] as PushPermission[]) {
        for (const optedOut of [false, true]) {
          for (const promptDismissed of [false, true]) {
            const action = decidePushSync({ hasSubscription, permission, optedOut, promptDismissed });
...
describe('flaggor i storage', () => {
  ...
  it('ett aktivt ja rensar både av-val och avfärdande', () => {
...
describe('shouldPersistEndpoint — en omPOST per session', () => {
  it('första gången ja, andra gången nej', () => {
    const s = fakeStorage();
