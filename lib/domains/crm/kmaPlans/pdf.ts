...
import type { KmaBlock, KmaDocument } from './types';

// KMA-planen som PDF — ritar ett sparat dokument (document.ts), block för block.
//
// Renderaren BYGGER INGEN TEXT. Allt som skrivs ut finns i dokumentet, och dokumentet sparas på
// varje revision; därför blir en omladdning samma innehåll som kunden fick. Här bor bara formen:
// typsnitt, avstånd, brytningar, sidhuvud och sidfot.
//
...
//   * Varje bilaga börjar på ny sida (breakPage — utan fortsättningshuvud).
//
...
function drawFooter(page: PDFPage, fonts: Fonts, doc: KmaDocument, index: number, total: number) {
 ...
 */
function layoutParagraph(block: Extract<KmaBlock, { t: 'p' }>, fonts: Fonts): ParagraphLayout {
...
function layoutTable(block: Extract<KmaBlock, { t: 'table' }>, fonts: Fonts): TableLayout {
 ...
 */
function leadingHeight(blocks: KmaBlock[], index: number, fonts: Fonts): number {
  ...
  switch (block.t) {
    ...
    case 'h3':
      ...
      // huvudrubriken stod ensam sist på sidan. Svepet i tests/crm/kmaPdf.test.ts fångade det.
...
type HeadingBlock = Extract<KmaBlock, { t: 'h1' | 'h2' | 'h3' }>;
...
function gapBefore(block: KmaBlock | undefined): number {
  if (!block) return 0;
...
function drawBlocks(blocks: KmaBlock[], fonts: Fonts, flow: Flow) {
...
async function embedLogo(pdf: PDFDocument, bytes: Uint8Array | null, kind: 'png' | 'jpg'): Promise<PDFImage | null> {
  ...
  try {
    ...
    console.warn('[kma-pdf] logotypen kunde inte bäddas in:', e instanceof Error ? e.message : e);
    return null;
  }
}
...
export async function renderKmaPdf(doc: KmaDocument, assets: KmaPdfAssets = {}): Promise<Uint8Array> {
  ...
  for (const [index, page] of pages.entries()) drawFooter(page, fonts, doc, index + 1, pages.length);

  const issued = isoDateToUtc(doc.meta.issuedOn);
  pdf.setTitle(`KMA-plan ${doc.meta.projectNumber} – ${doc.meta.projectName}`);
  pdf.setSubject(`Revision ${doc.meta.revision}`);
