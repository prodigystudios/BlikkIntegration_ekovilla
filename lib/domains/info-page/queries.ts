...
import { signInfoImageUrls } from './storage';
...
export async function loadInfoPage(client: SupabaseClient): Promise<InfoGroup[]> {
  const [groupsRes, sectionsRes, imagesRes] = await Promise.all([
    ...
    client
      ...
      .select('id, section_id, caption, file_name, storage_bucket, storage_path, public_path, sort_order, created_at')
      .order('sort_order')
      .order('created_at'),
  ]);
  ...
  for (const row of imageRows) {
    ...
    list.push({ id: row.id, caption: row.caption, fileName: row.file_name, url });
