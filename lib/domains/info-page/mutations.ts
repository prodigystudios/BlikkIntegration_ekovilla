...
import { isInfoImagePath } from './storage';
...
export async function registerImage(
  client: SupabaseClient,
  sectionId: string,
  input: { bucket: string; path: string; fileName: string; caption?: string | null },
) {
  ...
  const { data, error } = await client
    ...
    .select('id, file_name, caption, sort_order')
    .maybeSingle();
  failOn(error, 'image_create_failed');
  return requireRow(data, 'image_create_empty', 'Bilden registrerades inte.');
}
...
export async function deleteImage(client: SupabaseClient, id: string) {
  ...
  return requireRow(data, 'image_not_found', 'Bilden finns inte.') as {
    id: string;
